...
def beggs_robinson_mu_undersat(mu_b, P_psia, Pb_psia):
...
def grid_tune(pred, truth, bounds=(0.6, 1.6), steps=41):
    ...
    best_s, best_rmse, best_mape = 1.0, 1e99, 1e99
    for s in s_vals:
        rmse_val = rmse(truth[mask], s * pred[mask])
        mape_val = mape(truth[mask], s * pred[mask])
        if rmse_val < best_rmse:
            best_rmse, best_mape, best_s = rmse_val, mape_val, s
    return best_s, best_rmse, best_mape
...
# Sidebar
# -----------------------------
st.set_page_config(page_title="Módulo 2 — PVT (v4)", layout="wide")
st.title("Módulo 2 — PVT (v4): Autofit + Aceptar Ajuste")
...
with st.sidebar:
    ...
    st.divider()
    st.header("Autofit")
    base_for_rsb_from_pb = st.selectbox("Base para Rsb(Pb) y curvas μo",
                                        ["Standing", "Vasquez–Beggs", "Glaso"], index=0)
    st.caption("Nota: para 'Derivar Rsb desde Pb', esta base define Rs(Pb)=Rsb y μo.")
    do_autofit = st.button("🔧 Autofit con datos cargados")
    accept_fit = st.button("✅ Aceptar ajuste (guardar como configuración de proyecto)")
...
if mode == "Derivar Pb desde Rsb":
    ...
    source_note = "Pb derivado con Standing a partir de Rsb ingresado"
else:
    Pb = float(Pb_input)
    if base_for_rsb_from_pb == "Standing":
        Rsb = float(standing_rs(Pb, API, sg_gas, T_F))
        base_key = ("Rs","Standing")
    elif base_for_rsb_from_pb == "Vasquez–Beggs":
        Rsb = float(vb_rs(Pb, API, sg_gas, T_F))
        base_key = ("Rs","Vasquez–Beggs")
    else:
        Rsb = float(glaso_rs(Pb, API, sg_gas, T_F))
        base_key = ("Rs","Glaso")
    source_note = f"Rsb derivado con {base_for_rsb_from_pb} a partir de Pb ingresado"

if not (pmin < Pb < pmax):
    st.warning(f"Pb={Pb:.0f} psia está fuera del rango; ajusta pmin/pmax para verla.")
...
if show_mu:
    ...
    mu_base_name = base_key[1]
    mu_sat_curve = beggs_robinson_mu_sat(np.where(P <= Pb, Rs_pw[("Rs", mu_base_name)], Rsb), mu_dead)
    if add_usat:
        mu_b = beggs_robinson_mu_sat(Rsb, mu_dead)
        mu_usat = beggs_robinson_mu_undersat(mu_b, P, Pb)
        mu_series = np.where(P <= Pb, mu_sat_curve, mu_usat)
    else:
        mu_series = mu_sat_curve

# -----------------------------
# Datos de laboratorio (opcional)
# -----------------------------
st.subheader("Datos de laboratorio (opcional)")
uploaded = st.file_uploader("CSV con columnas P_psia, Rs_scfstb, Bo_rbblstb, mu_cp", type=["csv"])
lab_df = None
...
with col3:
    ...
    if mu_series is not None:
        ax3.plot(P, mu_series, label=f"μo (base: {base_key[1]})")
...
# -----------------------------
# Autofit (escala multiplicativa + vista ajustada)
# -----------------------------
fit_result = {"s_rs":1.0, "s_bo":1.0, "s_mu":1.0, "base": base_key[1], "Pb": float(Pb), "Rsb": float(Rsb),
              "co": float(co), "normalize_at_pb": bool(normalize_at_pb)}
...
if lab_df is not None and "P_psia" in (lab_df.columns):
    ...
    if do_autofit:
        st.markdown("### Vista *Autofit* (curvas base escaladas a laboratorio)")
        ...
        with colC:
            ...
            axC.plot(P, mu_series * fit_result["s_mu"], label="μo ajustada")
        ...
        st.write("**Métricas del autofit (base:**", base_name, ")")
        cols = st.columns(3)
        with cols[0]:
...
# -----------------------------
# Aceptar ajuste y guardar configuración
# -----------------------------
...
if accept_fit:
    cfg = {
        ...
        "base_correlation": fit_result["base"],
        "API": float(API),
        "sg_gas": float(sg_gas),
        "T_F": float(T_F),
        "Pb": float(Pb),
...
st.caption("Otros módulos pueden leer 'pvt_fit_config.json' para usar la correlación base y escalas aceptadas.")
